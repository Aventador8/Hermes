...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _sample_negative(self, Q):
        ...
        X_ = torch.zeros((class_num * cache_size, feat_size)).float().to(self.device)
        y_ = torch.zeros((class_num * cache_size, 1)).float().to(self.device)
        sample_ptr = 0
        for c in range(class_num):
            if c == 0:
                continue
            this_q = Q[c, :cache_size, :]

            X_[sample_ptr:sample_ptr + cache_size, ...] = this_q
            y_[sample_ptr:sample_ptr + cache_size, ...] = c
    ...
    def forward(self, feats, labels=None, predict=None, queue=None):
        # feats: batchsize * C(dim) * H * W        labels/predict: batchsize * H * W
