...
class preUnet(nn.Module):
    ...
    def forward(self, x, cls_model=None, cls_optimizer=None): # bs, 3, 224, 224
        if self.saliency_button:
            ...
            bridge = None
        x = self.resnet.conv1(x)   # bs, 64 , 112, 122
        x = self.resnet.bn1(x)
        ...
        if bridge is not None:
            x_up_1 = self.conv_up_2_1(torch.cat([x2, x_up_1, bridge], 1))  # 512, 28 28
        else:
            x_up_1 = self.conv_up_2_2(torch.cat([x2, x_up_1], 1))  # 512, 48, 48

        x_up_2 = self.conv_up_3(self.up(x_up_1))  # 256, 64, 64
        x_up_2 = self.conv_up_4(torch.cat([x1, x_up_2], 1))  # 256, 64, 64

        x_up_3 = self.conv_up_5(self.up(x_up_2))  # 64, 128, 128 112 112
        x_up_3 = self.conv_up_6(torch.cat([x, x_up_3], 1))  # 64, 128, 128 112

        x_up_4 = self.up(x_up_3)  # 64, 256, 256  224 224
        output = self.final(x_up_4)   # 3, 256, 256 224 224

        if self.consistency_button:
            consistency_feat = self.interTaskConsistency_seg(x_up_4)
        else:
